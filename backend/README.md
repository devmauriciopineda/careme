...
- One response envelope for every outcome, success or failure, plus a global
  exception handler that maps unexpected failures to `500`.
...
- **Request flow**: controller → service → `MeasurementJpaRepository` →
  ...
  `ApiResponse`.
- **Failure flow**: an unexpected failure surfaces through `ApiExceptionHandler`
  as HTTP 500. There is no 404 path any more, because no resource can go missing.
- **Invariants** live in the `Measurement` record's compact constructor, so an
  ...
  constraints and a unique constraint on `date`.
